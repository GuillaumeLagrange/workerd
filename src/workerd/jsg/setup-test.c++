...
KJ_TEST("configuration values reach nested type declarations") {
  {
    ConfigIsolate isolate(v8System, v8::IsolateGroup::Create(), 123, kj::heap<IsolateObserver>());
  ...
  }
  {
    ...
    ConfigIsolate isolate(v8System, v8::IsolateGroup::Create(), 456, kj::heap<IsolateObserver>());
    isolate.runInLockScope([&](ConfigIsolate::Lock& lock) {
      jsg::Lock& js = lock;
      js.withinHandleScope([&] { lock.newContext<ConfigContext>().getHandle(lock); });
    });
